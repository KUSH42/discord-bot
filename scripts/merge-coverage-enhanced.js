...
class CoverageMerger {
  ...
  async mergeLcovFiles(files, outputPath) {
    ...
    try {
      // Try using lcov-result-merger (preferred)
      const command = `npx lcov-result-merger ${files.map(f => `"${f}"`).join(' ')} "${outputPath}"`;
      console.log(`Executing: ${command}`);
      execSync(command, { stdio: 'pipe' });
      console.log('✅ Used lcov-result-merger for merging');
    } catch (error) {
      console.log('⚠️  lcov-result-merger failed, falling back to custom merger');

      // Fall back to our Python merger
      ...
      console.log('✅ Used Python merger as fallback');
    }
  }
